...
启动命令：
    python h1/run_step3_real.py

参数说明：
    --visible-devices：传给每个 cell 的 CUDA_VISIBLE_DEVICES。
    --reps：每个 budget/policy 工作点重复运行次数。
    --budgets：预算档位列表，默认 tight/mid/loose。
    --policies：策略列表，默认四策略。
    --max-requests：每个 cell 回放请求数。
    --replay-batch-size：单点运行时的回放批大小。
    --batch-sweep：改为扫描多个 replay_batch_size，并固定第一个 budget。
    --recommended-batch-sweep：使用脚本内 DEFAULT_BATCH_SWEEP。
    --batch-order：批内请求排序方式，original 或 length_bucket。
    --max-num-batched-tokens：vLLM max_num_batched_tokens。
    --warmup-batches：正式计量前跳过的 warmup batch 数。
    --sharegpt-path：ShareGPT 原始 JSON 路径，用于冻结 mixed trace。
    --hotpotqa-path：本地 HotpotQA 数据目录。
    --replay-trace：冻结后的 replay trace 输出/输入路径。
    --force：已有输出时仍重跑，并重建 trace。
    --keep-cells：保留每轮 cell 输出和日志。
"""
