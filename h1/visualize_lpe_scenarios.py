#!/usr/bin/env python3
"""Visualize the LPE_NOT_GOOD A/B/C scenario comparison (Python / matplotlib).

Reads the Step3 repeat-protocol summary (cross-rep medians) produced by
run_step3_repeat.py / summarize_step3_repeat.py and draws a 4-policy comparison
(vLLM / LRU / LFU / LPE) across the three scenarios:

    A  press_0720_n200   0.720 / n=200   pressured non-saturated (primary)
    B  press_0730_n200   0.730 / n=200   non-saturated
    C  sat_0710_n400     0.710 / n=400   saturated

Usage (no env vars required):
    python h1/visualize_lpe_scenarios.py
    python h1/visualize_lpe_scenarios.py --summary <path> --out <png>

Saves <out>.png and <out>.pdf next to the summary CSV by default.
"""
...
# fixed ordering: scenarios A,B,C and policies vLLM,LRU,LFU,LPE
