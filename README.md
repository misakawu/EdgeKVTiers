...
> H1 实验驱动（原 `h1-pre/`）已并入 `h1/`，详见 `h1/H1_readme.md`。
...
H1 的高层实验脚本会通过 `conda run -n edgekv-vllm0110` 启动真实 vLLM cell；通常不需要先手动激活环境。先用 dry-run 检查命令和输出目录：
...
如果要直接运行底层 real-replay harness，需要显式使用 H1/H0 的 `PYTHONPATH`：
...
--replay-batch-size                   replay 批大小/并发代理，real-replay 推荐从 8/16/32/64 扫描
...
# 并发扫描:固定单一 budget,定位队列排队和策略差异的平衡点：
python3 h1/run_step3_real.py --batch-sweep 8 16 32 64 --visible-devices 0,1
```

trace 由 `h0/build_h0_replay_trace.py` 生成并默认保存在 `data/edgekv_traces/`。输出与汇总在 `h1/out/step3_real/`
（`step3_real_summary.csv`）。重点看 `queue_wait_ratio_mean`、`queue_wait_p95_ms`、`prefill_p95_ms` 和
`batch_queue_span_p95_ms`，判断 TTFT proxy 主要来自 batch 内排队还是 prefill。
